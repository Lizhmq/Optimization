...
def bfgs(x, func, grad_func, Hk=np.identity(2)):
    grad_list, f_list = [], []
    d_list, x_list = [], []
    H_list = []
    while True:
        ...
        if len(grad_list) > 1:  # rank-1 correction
            ...
            Hk = Hk + (1 + np.dot(deltag, mid) / np.dot(deltag, deltax)) * np.outer(deltax, deltax) / np.dot(deltax, deltag)
            Hk = Hk - (np.outer(mid, deltax) + np.outer(mid, deltax).T) / np.dot(deltag, deltax)
        H_list.append(Hk)
...
x = np.zeros(3)
gl, fl, xl, Hl = bfgs(x, func2, grad_func2, Hk=np.identity(3))
print(len(gl))
print(xl)